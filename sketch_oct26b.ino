...
void setup(void) {
  ...
  server.on("/", HTTP_GET, []() {
    ...
    html += "input[type='submit'] { background-color: #4CAF50; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; transition: background-color 0.3s ease; }";
    html += "input[type='submit']:hover { background-color: #45a049; }";
    html += "#turnOffButton { background-color: #FF5722; }";
    html += "#turnOffButton:hover { background-color: #E64A19; }";
    html += "</style>";
    html += "</head>";
    html += "<body>";
